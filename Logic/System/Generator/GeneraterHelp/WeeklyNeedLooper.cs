...
namespace Logic.System.Generator
{
    public class WeeklyNeedLooper : IGetLoopInfoWeeklyNeed
    {
        ...
        /// <param name="week">The date to get the weekly need for.</param>
        ...
        public Result<IWeeklyNeed> GetInfo(Company company, int week)
        {
            try
            {
                ...
                if (weeklyNeed == null)
                {
                    return Result<IWeeklyNeed>.Fail(
                        new Exception("there doesnt exist a weeklyneed with the given date")
                    );
                }
